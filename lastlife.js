...
const { fermerLeTicketApres } = require("./config.json");
const fs = require("fs");
...
client.ticket_cooldown = fermerLeTicketApres;
...
async function RegisterSlashCommands() {
	try {
		let apiRoutings = process.env.MODE === "dev" ? Routes.applicationGuildCommands(process.env.APP_CLIENT_ID, process.env.APP_GUILD_ID) : Routes.applicationCommands(process.env.APP_CLIENT_ID);

		await rest.put(apiRoutings, { body: commandJsonData });
