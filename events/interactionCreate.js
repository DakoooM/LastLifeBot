    ...
    **/
    async execute(client, interaction) {
        if (interaction.isChatInputCommand()) {
            ...
            if (interaction.customId === "customTicket") {
                for (let i = 0; i < tickets.length; i++) {
                    let item = tickets[i];

                    if (interaction.values[0] === item.customId) {
                        console.log(colors.red("stop check"), item.customId);
                        ticketInteraction(client, interaction, item.customId);
                        break;
                    }

                    console.log("CUSTOMID →", item.customId);
                }
            }
        }
    }
