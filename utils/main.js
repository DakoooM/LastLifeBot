...
class Util {
    ...
    */
     ...
     * @param {Object} param Paramètres pour récuperer le rôle depuis le bot
     * @returns si un role exist alors cela renverra l'object du rôle
    */
    async getRoles(param) {
        return await (this.client ? this.client : param.client).guild.roles.cache.get(param.id);
    }

    async fetchPlayers(ip, port = 30120) {
        let pData = [];
        let pStatus = 404;

        try {
            const result = await axios.get(`http://${ip}:${port.toString()}/players.json`);
            pData = result?.data;
        ...
        return [pStatus, pData];
    }

    getInfoTicket(customId, checkCategory = false) {
        for (let i = 0; i < tickets.length; i++) {
            let item = tickets[i];

            if (checkCategory) {
                if (customId === item.categoryId) return item; 
            } else {
